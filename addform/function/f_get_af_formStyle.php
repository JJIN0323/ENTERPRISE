<?
function f_get_af_formStyle($upload_path,$name)			//업로드 경로,폼이름
{

#########################################################################################
######################	업로드 폴더에서 스타일시트 가져오기			#####################
#########################################################################################
	if(file_exists($upload_path.$name."/formStyle.css"))//사용자스타일이 있을 경우 읽기모드로 가져옴
		{
		$css_path = $upload_path.$name."/formStyle.css";
		if(filesize($css_path) == 0) $fs = "100";		//filesize(경로)에서 0 일 경우 에러방지위해...
		else $fs = filesize($css_path);				
		$fp = fopen($css_path,"r");
		$formStyle = fread($fp,$fs);
		}
	 else $formStyle = "";	
...
/* link 스타일 */
A:link{TEXT-DECORATION:none;COLOR:#3260AD;}
A:hover{TEXT-DECORATION:none;COLOR:#004A84;BORDER:none;}
A:visited{TEXT-DECORATION:none;COLOR:#616588;BORDER:none;}
...
/* 전체폼기본환경설정에서 폼분류 선택상자 표시 여부를 no에 체크하면 */
/* 품분류선택상자는 폼상단에 나타나지 않습니다. */
/***************************************************************************************/
div.groupANDform{
margin-bottom:20px;border:2px solid #6581a2;background-color:#f3f4f3;text-align:left;
...
/***************************************************************************************/
/* 폼형식이 폼메일일 때 사용될 스타일 정의 */
/***************************************************************************************/

/* 상단 배너이미지 td 스타일(폼메일, 주문폼 공통) */
td#top_img {text-align:left;}

/* 폼메일 제목 div 스타일 */
div#form_title {text-align:left;border-bottom:3px solid #f0f0f0;margin-top:20px;}   

/* 폼메일 제목 h3 스타일  */
div#form_title h3{font-size:1.2em;font-weight:bold;border-bottom:1px solid #bfbfbf;padding-bottom:10px;margin:0;color:#333;}

/* 상단에 표시될 글  div 스타일 (폼메일, 주문폼 공통) */
div#msg_top {text-align:left;margin:10px 0 10px 0;}

/* 폼메일 항목들 테이블과 td 스타일 */
table#formmail_items{width:100%;}
#formmail_items td{border:1px solid #d6d6d6;padding:3px 5px 2px 5px;}

/* 단락구분에 사용되는 td의 스타일 */ 
#formmail_items .af_divi{margin:0;padding:20px 0 5px 0;border:0;border-bottom:3px solid #bbbbbb;text-align:left;}

/* 폼메일 항목 td 스타일 */
#formmail_items td.item{background-color:#e9e9e9;background-repeat:repeat-x;text-align:left;border-left:1px solid #cccccc;font-size:11px;font-weight:bold;color:#333;}

/* 폼메일 항목 오른쪽 입력필드 td 스타일 */ 
#formmail_items td.val{text-align:left;}

/* 폼메일 항목 간단설명 스타일 */
#formmail_items .item_tip{color:#999;font-size:11px;line-height:1.2em;margin:0;padding:0;}

/* 폼메일 고객주소입력 테이블 */
#formmail_items .c_address {width:100%;}
#formmail_items .c_address td{border:0;padding-left:0;}
#formmail_items .c_address input{}

/* 마우스가 올라갔을 때, 변하는 행 배경색상 */
tr.roll_table{background-color:#f5f5f2;}

/* 필수입력필드 배경색상 */
INPUT.ess{background-color:#FAFAD2;}
TEXTAREA.ess{background-color:#FAFAD2;}
...
/* 폼메일 테이블 하단 전송버튼 바로위 마감 테두리 스타일 */
...
/* 물품상세이미지 div 스타일(폼메일, 주문폼 공통) */
div#detail_img {line-height:1.3em;margin-top:20px;}

/* 하단에 표시될글 div 스타일(폼메일, 주문폼 공통) */
div#msg_bottom{text-align:center;color:gray;}


/***************************************************************************************/
/* 폼형식이 주문폼일 때 사용될 스타일 정의 */
/***************************************************************************************/
...
/* 주문폼 제목 p 문단 스타일 */
p#f_title {font-size:16px;font-family:굴림;font-weight:bold;height:40px;padding:12px 0 0 80px;background-repeat:no-repeat;margin:5px 0 5px 0;}
