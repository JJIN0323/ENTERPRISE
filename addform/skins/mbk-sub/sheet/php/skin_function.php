...
function f_SelectBox($arr,$subject,$coin_unit)
{
	...
	if($af_TABLE5["secret_price"] == 1)	$attach_talil; 		 //  = "&nbsp;&nbsp;+ $coin_unit" 떼어냄 - +원 으로 표시되는것.
...
function f_CheckBox($arr,$subject,$coin_unit,$i) 
{
	...
	if (count($arr[text]) > 0) {							 //text 배열의요소가 1개이상일 경우
		...
		{			
			...
			{
				$buf.="<input type='checkbox' ".$arr[name]." ".$arr[id]." ".$script." value='".$arr[text][$i]."|".$arr[optVal][$i]." 'style='width:15px; border:0; height:13px;' ".$checked." >".$arr[text][$i]."\n";
			}            
        }
    }
...
function f_Radio($arr,$subject,$coin_unit,$i) 
{
	...
	if (count($arr[text]) > 0) {							 //text 배열의요소가 1개이상일 경우
        for ($i=0;$i < count($arr[text]);$i++) 
		{
			...
			{
				 $buf.="<input type='radio' ".$arr[name]." ".$arr[id]." ".$script." value='".$arr[text][$i]."|".$arr[optVal][$i]." 'style='width:15px; border:0; height:12px;' ".$checked.">".$arr[text][$i]."\n";
			}           
        }
    }
