<?php
	...
	<div id="submit">
		<table <?php if($_GET["mode"] == "admine") echo "style='display:none;'";?>>
			<tr>
				<!--
				<td style="border:1px solid #b9b39e;">
					
					<!-- 전체폼 기본환경 설정에서 설정한 사업자 정보 --
						<ul>
							<li>
								<span>상호 : <?php echo $af_TABLE1["supply_name"];?></span> ▣ 
							<?php if($af_TABLE1["supply_num"]){?>
								<span>사업자등록번호 : <?php echo $af_TABLE1["supply_num"];?></span> ▣ 
							<?php } ?>
								<span>대표 : <?php echo $af_TABLE1["supply_man"];?></span> ▣ 
								<span>주소 : <?php echo $af_TABLE1["supply_address"];?></span> ▣  
								<span style='font-weight:bold;'>대표전화 : <?php echo $af_TABLE1["tel"];?></span> ▣ 
							<?php if($af_TABLE1["fax"]){?>
								<span>팩스 : <?php echo $af_TABLE1["fax"];?></span> ▣ 
							<?php } ?>
							<?php if($af_TABLE1["sell_num"]){?>
								<span>통신판매업신고번호 : <?php echo $af_TABLE1["sell_num"];?></span> ▣ 
							<?php } ?>
								<span>전자우편 : <?php echo $af_TABLE1["email_address"];?></span> 
							</li>	
						</ul>
					
				</td> -->
								
				<td style="width:250px;text-align:right;" class="print">
				<?php if($_GET["mode"] != "admine") {//관리자모드가 아닐 때?>
						...
						$af_arr_format = array("jpg","jpeg","gif");				//업로드 된 허용확장자
							...
							if($token_chk_btnImg == 0)
								{
									echo "<input type=button value='".$arr_dummy13[3]."' name='reentery' onclick='reset();' class='btn_big'>";								
								}
						?>					
				<?php }?>				
				</td>
