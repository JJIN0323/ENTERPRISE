function chk_readN_no(frm,level,read_order_level,msg) 				//변경시 선택개수 체크 함수
{
	...
	{		
		...
		if(total < 1)
		{
			alert("읽지않은상태로 표시할 항목을 1개 이상 선택하십시오.\n\n please select at least one item");
		}
		else
		{
			chk_read_no(frm);
...
function chk_read_no(frm) 
{																	//확인창 팝업
	checking=confirm('선택한 항목을 읽지않은상태로 변경하시겠습니까?\n\n To display no read the selected items?');
	if (checking==true)
	{			  
		frm.mode.value="read_no";									//모드 변경
		frm.submit();
	}
	else return false;	
}